...
# Read metadata
meta = read.table("Input/metadata.csv", header=T, sep=";")
...
ps.sub <- subset_samples(ps, Diet %in% c("Low","High","Control","Blank"))
ps.sub <- subset_samples(ps.sub, Animal %in% c("123","127","134","136","138","142","Control","Blank"))
ps.con <- ps.sub

#Excluding barcodes of control, as it is empty - ikke kørt
ps.sub=prune_samples(!(sample_names(ps.sub) %in% c("barcode01","barcode17","barcode33","barcode48","barcode64","barcode79","barcode80","barcode95","barcode96")), ps.sub)
#ps.sub1=prune_samples(!(sample_names(ps.sub) %in% c("barcode95","barcode96")), ps.sub)
...
p + facet_wrap( ~ Animal  , nrow = 1, scales = "free_x" ) +
  theme(
...
p2 <- plot_ordination(
  ...
  geom_point(size = 1) +   # bigger dots
  geom_text_repel(
    aes(label = ID),
    size = 2,               # text size
    nudge_x = 0.01,          # horizontal offset
    nudge_y = 0.01,          # vertical offset
    segment.color = NA      # removes the line
  ) +
  theme(
    text = element_text(size = 12),
...
## Get nested top taxa ---- The top five 5 ranked taxa are chosen
top_nested.con <- nested_top_taxa(
  ps.con,
  top_tax_level   = "Order",
  nested_tax_level = "Species",
  n_top_taxa      = 5,
...
sample_labels=sample_data(ps.con)$ID

p2 + facet_wrap(~ Animal, nrow = 1, scales = "free_x") +
  theme(
    axis.text.x = element_text(size=12, angle = 90, hjust = 1),
    axis.text.y = element_text(size=12),
    strip.text  = element_text(size = 16, face = "bold"),
...
ps.ord <- ordinate(ps.con, method = "NMDS", distance = "bray")
...
dist.bc2 <- phyloseq::distance(ps.con, method = "bray")

meta$Group1 <- ifelse(meta$Diet %in% c("Control", "Blank"),
                     "Control_Blank",
                     "Diet")
adonis2(dist.bc2 ~ Group1, data = meta)

meta$Group2 <- factor(meta$Diet,
                      levels = c("Blank", "Control", "Low", "High"))
adonis2(dist.bc2 ~ Group2, data = meta)


###### ---- Ensure NO transformation to relative abundance ----
